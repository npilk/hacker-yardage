...
def drawGreenDistancesTree(image, adjusted_hole_array, tree_list, ypp, text_size, text_color, filter_dist=25, par_3_tees=0):
    ...
    for point in distance_points:
        ...
        if distance % 50 < 7 or distance % 50 > 43:
            close = True
        ...
        if right:
            ...
            # don't draw a line through the text!

            line_drawpoint = (point[0] - 75,point[1])

            cv2.line(image,(int(point[0]),int(point[1])),(int(line_drawpoint[0]),int(line_drawpoint[1])),text_color, 3)
